...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!userRes.ok) {
      console.error(`[password/reset] Failed to find user with reset token:`, userRes.status);
      return NextResponse.json(
    ...
    if (!updateRes.ok) {
      ...
      console.error(`[password/reset] Failed to update password for user ${user.id}:`, {
